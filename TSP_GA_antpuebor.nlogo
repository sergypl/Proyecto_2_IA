...
to AI:Compute-fitness ;contra mayor es el fitness mejor
  ...
  let res todos-weight content
  ;print (word "content: " content)
  ...
  set fitness   -1 * ( res + (N - length lista * 100) )
end
...
to AI:ExternalUpdate
  ...
  foreach c[
    ...
    if i = 9 [ ask cities with [ id = item i c] [ create-blue-link-with  one-of cities with[ id = item 0 c][
      set color blue
      ...
      set label-color red]  ]]
    if i != 9 [ask cities with [ id = item i c] [ create-blue-link-with  one-of cities with[ id = item (i + 1) c][
      set color blue
...
;    create-blue-link-with one-of other cities [
;      set color blue
...
;    ]
  ...
  ; ponemos que la posicion se conecte con el siguinte id de la tortuga
end
...
to-report todos-weight[ lista ]
  ...
  foreach lista[
    i ->
    if i = 9 [ set res res + compute-weight (item i lista) (item 0 lista) ]
    if i != 9 [ set res res + compute-weight (item i lista) (item (i + 1) lista) ]
  ]
